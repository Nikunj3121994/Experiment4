...
(function (global, doc, undefined) {
    ...
    var PageVisibility = function (onFocus, onBlur) {
        ...
        if (!_visibilityChange && doc.onfocusin === undefined)
            return;
        /**
    ...
    p.destroy = function () {
        // If this browser doesn't support visibility
        if (!_visibilityChange || !this._onToggle)
            return;
        this.enabled = false;
        this._onToggle = null;
        this._onFocus = null;
...
(function (undefined) {
    ...
    SavedData.read = function (name) {
        if (WEB_STORAGE_SUPPORT) {
            ...
            var nameEQ = name + "=", ca = document.cookie.split(';'), i = 0, c, len;
            for (i = 0, len = ca.length; i < len; i++) {
                c = ca[i];
                while (c.charAt(0) == ' ')
                    c = c.substring(1, c.length);
                if (c.indexOf(nameEQ) === 0)
                    return JSON.parse(unescape(c.substring(nameEQ.length, c.length)), SavedData.reviver);
            }
            return null;
        }
    };
...
(function (document, undefined) {
    //Import classes
    var EventDispatcher = include('springroll.EventDispatcher'), Features = include('springroll.Features'), Bellhop = include('Bellhop'), $ = include('jQuery');
    /**
...
(function () {
    ...
    plugin.setup = function () {
         ...
         * @private
         */
        this._captionsStyles = Object.merge({}, DEFAULT_CAPTIONS_STYLES, SavedData.read(CAPTIONS_STYLES) ||
            {});
        /**
...
(function () {
    ...
    plugin.setup = function () {
         ...
         * @private
         */
        this._pageVisibility = new PageVisibility(onContainerFocus.bind(this), onContainerBlur.bind(this));
        /**
